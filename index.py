...
def tampilkan_informasi_pembelian(nama_pelanggan, tanggal_pembelian, pesanan, total_pembayaran, uang_dibayar, kembalian):
    print()
...
def pesanan_baru():
    ...
    while True:
        ...
        if menu_pesanan in [1, 2, 3, 4]:
            if any(item['menu'] == menu_pesanan for item in pesanan):
                index = next((index for index, item in enumerate(pesanan) if item['menu'] == menu_pesanan), None)
                jumlah_t = int(input('Masukkan jumlah pembelian : '))
                pesanan[index]['jumlah'] += jumlah_t
                ...
                if menu_pesanan == 1:
                    ...
                    harga = 17000
                pesanan.append({'menu': menu_pesanan, 'nama': 'Ayam Mayo' if menu_pesanan == 1 else 'Ayam Katsu' if menu_pesanan == 2 else 'Ayam Koloke' if menu_pesanan == 3 else 'Ayam Bakar', 'harga': harga, 'jumlah': jumlah_t})
                print('Pesanan berhasil ditambahkan!')
            break
        else:
    ...
    while True:
        ...
        if question.lower() == 'tambah':
            ...
            if tambah in [1, 2, 3, 4]:
                if any(item['menu'] == tambah for item in pesanan):
                    index = next((index for index, item in enumerate(pesanan) if item['menu'] == tambah), None)
                    jumlah_t = int(input('Masukkan jumlah tambahan : '))
                    pesanan[index]['jumlah'] += jumlah_t
                    print('Pesanan berhasil ditambahkan!')
                else:
                    jumlah_t = int(input('Masukan jumlah pembelian : '))
            ...
            tampilkan_informasi_pembelian(nama, tanggal, pesanan, total_pembayaran, uang_dibayar, kembalian)
            break
        else:
